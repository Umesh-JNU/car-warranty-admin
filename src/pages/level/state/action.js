...
export const getAll = async (dispatch, token, curPage, resultPerPage, query) => {
  let url = `/api/admin/levels/?keyword=${query}&resultPerPage=${resultPerPage}&currentPage=${curPage}`;
...
export const del = async (dispatch, token, id) => {
  if (
    ...
    try {
      dispatch({ type: "FETCH_REQUEST" });
      await axiosInstance.delete(`/api/admin/level/${id}`, {
        headers: { Authorization: token },
      });
...
export const update = async (dispatch, token, id, levelInfo) => {
  try {
    dispatch({ type: "UPDATE_REQUEST" });

    await axiosInstance.put(`/api/admin/level/${id}`, levelInfo, {
      headers: { Authorization: token },
    });
...
export const getDetails = async (dispatch, token, id) => {
  ...
  try {
    ...
    const { data } = await axiosInstance.get(`/api/admin/level/${id}`, {
      headers: { Authorization: token },
    });
